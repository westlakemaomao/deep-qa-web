...
$(document).ready(function(){
	...
	//调用qa-web接口获取问答结果
	function getQAwebMesaage(message) {
		console.info("query:"+message)
		var answer;
		$.ajax({
			dataType : "json",
			url : 'http://localhost:8080/deep-qa-web/api/ask?n=1&q='+message,
			type : 'GET',
			async : false,
...
//	var json=getQAwebMesaage(message);
//	console.info("result:"+json);
	...
	//-----------message send end--------------------
		...
		messages++;
      //调用返回消息
		if(Math.random()<0.65 || lastMessage.indexOf("?")>-1 || messages==1) getReply(message);
	}
	
	//返回消息
	function getReply(requestStr){
		console.info("getReply:"+requestStr)
		...
		//----------handle the message answer start-----------------
		var message="";
		var json=getQAwebMesaage(requestStr);
